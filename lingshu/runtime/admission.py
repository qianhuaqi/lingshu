...
class BoundedAdmission:
    ...
    async def acquire(self, *, deadline: Deadline | None = None) -> AdmissionLease:
        ...
        if deadline is not None and deadline.expired(self.clock):
            raise _admission_error(
                "admission.timeout",
                "Admission wait deadline was exceeded.",
                retryable=True,
            )
        ...
        if deadline is None:
            raise _admission_error(
                "admission.deadline_required",
                "Queued admission requires a finite Deadline.",
            )
        ...
        return AdmissionLease(
            self,
            outcome=AdmissionOutcome.WAITED,
            waited_ns=max(0, self.clock.now_ns() - waiter.enqueued_ns),
        )
    ...
    def set_draining(self, draining: bool = True) -> None:
        ...
        if draining:
            self._reject_waiters(
                _admission_error(
                    "admission.draining",
                    "The service is draining and is not accepting new work.",
                    retryable=True,
                )
            )
    ...
    def set_healthy(self, healthy: bool) -> None:
        ...
        if not healthy:
            self._reject_waiters(
                _admission_error(
                    "admission.unhealthy",
                    "Admission dependency is unhealthy.",
                    retryable=True,
                )
            )
    ...
    def _waiter_was_granted(self, waiter: _Waiter) -> bool:
        return (
            ...
            and waiter not in self._waiters
        )
