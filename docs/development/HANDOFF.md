...
Branch: `human/dodo/p2-00-phase-roadmap`
...
- core wall/monotonic time helpers, typed identifiers, framework errors, safe Problem Details, and redaction primitives;
- immutable static configuration snapshot sufficient for startup;
- runtime deadlines, scopes, cancellation, task ownership, admission, bounded cleanup, and minimum Runtime Record behavior;
- HTTP value model with methods, targets, headers, bounded request body, response factories, and commit-state safety;
- router and middleware compilation with deterministic ordering and scope-bound `call_next`;
- `LingShu` application kernel with route decorators, immutable freeze, lifecycle, exception mapping, and root facade exports;
- native single-worker HTTP/1.1 server with bounded connection handling, parser limits, safe errors, drain, and close;
- CLI `version`, `check`, and `run --workers 1`;
- integration, security redaction, runtime watermark, packaging, example, and P1 acceptance evidence.
...
The final P1 CI matrix passed quality, governance, build, clean install, artifact validation, and cross-platform pytest jobs before merge.
...
5. Later Issues may evaluate reload/watch or multi-worker only after explicit ADR/task decomposition.

## Protected facts

- P1 remains a single-worker minimum vertical slice, not a production-ready stable release.
- Public PyPI publication remains unauthorized.
- Multi-worker supervisor, reload/watch, ASGI/WSGI/WebSocket/HTTP2/HTTP3, config hot reload, Auth/Tenant/RBAC/DB/OpenAPI integration, and production hardening remain deferred until explicitly authorized by later Issues and ADRs.
...
Open a Draft PR for Issue #78 from `human/dodo/p2-00-phase-roadmap`, verify documentation scope and CI, then wait for project-lead final merge review.
