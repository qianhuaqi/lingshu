# MySQL Extension Contract

Status: draft for P5-02 review
Context: Issue #118

## 1. Purpose

This document defines the MySQL data-extension track for LingShu. It records the
contract boundary the eventual MySQL extension must satisfy without implementing a
MySQL client in `lingshu` core.

P5-01 established the first data-extension reference pattern. P5-02 follows the
same contract-first shape while remaining MySQL-specific.

## 2. Scope

P5-02 is document-first. A minimal import-safe skeleton is optional and only
appropriate if later work needs a testable public surface.

This track does not:

- implement MySQL runtime behavior;
- add a mandatory MySQL dependency to `lingshu` core;
...
The eventual official MySQL extension package is expected to follow the P4
...
The future package name is expected to be `lingshu-mysql`, but this issue does
not require publishing or installing that package.

## 4. Configuration Contract

The MySQL extension should accept a small configuration schema that can be
projected into redacted diagnostics before any output leaves extension internals.

Suggested configuration fields:

- `dsn` or equivalent connection locator;
- `host`;
- `port`;
- `username`;
- `password`;
...
- DSNs are never shown in `repr`, logs, or `safe_details`;
- usernames, passwords, tokens, host credentials, database names, schema names,
  and query text are never shown in `repr`, logs, or `safe_details`;
...
The MySQL extension must follow the accepted P4 lifecycle rules:
...
MySQL extension diagnostics must remain safe by default:
...
- `repr` output must not expose DSNs, usernames, passwords, tokens, host
  credentials, database names, schema names, or query text;
...
- no full MySQL client;
- no production connection pooling;
- no migration framework;
- no ORM integration;
- no query builder;
- no read/write split or replication behavior;
- no Redis or MongoDB implementation;
...
## 8. P5-02 Acceptance Shape

P5-02 is complete when the MySQL track is documented clearly enough that later
implementation work can stay inside the accepted P4 contracts and the P5-01
reference pattern.
